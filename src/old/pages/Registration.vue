			 ...
			 <span class="title2">форма регистрации</span>
				 <form @submit.prevent="onSubmit">
					 ...
					 <div class="form-item" :class="{ errorInput: $v.email.$error }">
						 <input type="email"
						 placeholder="Почтовый ящик" v-model="email"
					 ...
					 <div class="form-item" :class="{ errorInput: $v.password.$error }">
						...
						<div class="error" v-if="!$v.password.required">поле пароль обязателен для заполнения</div>
					 ...
					 <div
						...
						:class="{ errorInput: $v.repeatPassword.$error }">
						 <input
							...
							v-model="repeatPassword" :class="{ error: $v.repeatPassword.$error }"
							@change="$v.repeatPassword.$touch()"/>
						 <div class="error" v-if="!$v.repeatPassword.sameAsPassword">оба пароля должны быть идентичными!</div>
					 </div>
					 ...
					 <div class="buttons-list buttons-list--info">
						 <p class="typo__p" v-if="submitStatus === 'OK'">спасибо за регистрацию <br> Сейчас перенаправим на главную страницу</p>
						 <p class="typo__p" v-if="submitStatus === 'ERROR'">пожалуйста, заполните поля корректно</p>
						 <p class="typo__p" v-else="v-else">submitStatus: {{ submitStatus }}</p>
					 </div>
					 <div class="buttons-list buttons-list--info">
						 <span>уже зарегистрированы?
							<p>на <router-link class="link" to="/login">страницу логина</router-link></p>
    ...
    onSubmit () {
      ...
      // Valid
        ...
        const user = {
		 ...
		 console.log('this.submitStatus: ' + this.submitStatus, 'email: ' + user.email + ' pass: ' + user.password);
		...
		//  комммитим мутацию 'registerStart'
